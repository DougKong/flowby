...
angular.module('mean.schedule').controller('ScheduleController', ['$scope', function ($scope){
    var tasks = [
...
angular.module('mean.schedule').directive('ghVisualization', function () {
  var taskStatus = {
    "SUCCEEDED" : "bar",
    "FAILED" : "bar-failed",
    "RUNNING" : "bar-running",
  ...
  var gantt = function(eleName, tasks) {
      ...
      d3.gantt(eleName)(tasks).taskTypes(taskNames);
    // d3.gantt(eleName).taskTypes(taskNames).taskStatus(taskStatus).tickFormat(format);
  };
